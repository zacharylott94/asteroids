...
function AsteroidSpawnSystem(objectList: any, difficulty: number) {
  if (objectList.asteroids.length < difficulty * 3)
    objectList.asteroids.push(LargeAsteroidFactory(generateSpawnLocation(), generateRandomVelocity()))



  function generateSpawnLocation(): TVector {
    const player = objectList.player[0]
